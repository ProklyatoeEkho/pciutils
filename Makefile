...
all: lib/$(PCILIB) lspci$(EXEEXT) setpci$(EXEEXT) example$(EXEEXT) nvapeek$(EXEEXT) nvapoke$(EXEEXT) nva_counters_config$(EXEEXT) lspci.8 setpci.8 pcilib.7 update-pciids update-pciids.8 $(PCI_IDS)
...
nvapeek$(EXEEXT): nvapeek.o common.o lib/$(PCILIB)
	$(CC) $(LDFLAGS) -o $@ $+ $(LDLIBS)
nvapoke$(EXEEXT): nvapoke.o common.o lib/$(PCILIB)
	$(CC) $(LDFLAGS) -o $@ $+ $(LDLIBS)
nva_counters_config$(EXEEXT): nva_counters_config.o common.o lib/$(PCILIB)
	$(CC) $(LDFLAGS) -o $@ $+ $(LDLIBS)
...
nvapoke.o: nvapoke.c pciutils.h $(PCIINC)
nva_counters_config.o: nva_counters_config.c pciutils.h $(PCIINC)
...
clean:
	...
	rm -f update-pciids lspci$(EXEEXT) setpci$(EXEEXT) example$(EXEEXT) nvapeek$(EXEEXT) nvapoke$(EXEEXT) nva_counters_config$(EXEEXT) lib/config.* *.[78] pci.ids.* lib/*.pc lib/*.so lib/*.so.*
	rm -rf maint/dist
