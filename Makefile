...
all: lib/$(PCILIB) lspci$(EXEEXT) setpci$(EXEEXT) example$(EXEEXT) nvapeek$(EXEEXT) nvapoke$(EXEEXT) lspci.8 setpci.8 pcilib.7 update-pciids update-pciids.8 $(PCI_IDS)
...
clean:
	...
	rm -f update-pciids lspci$(EXEEXT) setpci$(EXEEXT) example$(EXEEXT) nvapeek$(EXEEXT) nvapoke$(EXEEXT) lib/config.* *.[78] pci.ids.* lib/*.pc lib/*.so lib/*.so.*
	rm -rf maint/dist
